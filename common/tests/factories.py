...
def make_tenant(
    email: str = "tenant@example.com", password: str = "TestPass123!"
) -> User:
    """:return: a User with is_owner/is_agent False and an ACTIVE TenantProfile."""
    user = User.objects.create_user(
        email=email, password=password, phone=_unique_phone()
    )
    TenantProfile.objects.create(
        user=user, passport_data="P1234567", status=ProfileStatusChoices.ACTIVE
    )
    return user
...
def make_owner(
    ...
    """
    user = User.objects.create_user(
        email=email, password=password, phone=_unique_phone(), is_owner=True
    )
...
def make_certified_agent(
    email: str = "agent@example.com", password: str = "TestPass123!"
) -> User:
    """:return: a User with is_agent=True and an ACTIVE, certified AgentProfile."""
    user = User.objects.create_user(
        email=email, password=password, phone=_unique_phone(), is_agent=True
    )
