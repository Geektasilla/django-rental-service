...
class TicketAssignmentTests(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.busy_agent = User.objects.create_user(
            email="busy@example.com", password="TestPass123!", phone="+491700000001", is_support=True
        )
        self.free_agent = User.objects.create_user(
            email="free@example.com", password="TestPass123!", phone="+491700000002", is_support=True
        )
        # Give the busy agent an existing open ticket so the free agent has fewer.
        Ticket.objects.create(
            user=self.opener, assigned_to=self.busy_agent, subject="Existing", status=Ticket.StatusChoices.OPEN
        )
    ...
    def test_new_ticket_assigned_to_agent_with_fewest_open_tickets(self) -> None:
        response = self.client.post(
            "/api/v1/support/", {"subject": "Need help", "status": "open"}, format="json"
        )
    ...
    def test_ticket_left_unassigned_when_no_agent_exists(self) -> None:
        ...
        response = self.client.post(
            "/api/v1/support/", {"subject": "No agents available", "status": "open"}, format="json"
        )
        self.assertEqual(response.status_code, 201, response.data)
        ticket = Ticket.objects.get(pk=response.data["id"])
...
class TicketVisibilityTests(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.agent = User.objects.create_user(
            email="agent@example.com", password="TestPass123!", phone="+491700000003", is_support=True
        )
        self.ticket = Ticket.objects.create(
            user=self.opener, assigned_to=self.agent, subject="Private issue", status=Ticket.StatusChoices.OPEN
        )
        self.client = APIClient()
    ...
    def test_assigned_agent_can_see_and_reply_to_the_ticket(self) -> None:
        ...
        reply = self.client.post(
            f"/api/v1/support/{self.ticket.pk}/messages/", {"body": "How can I help?"}, format="json"
        )
