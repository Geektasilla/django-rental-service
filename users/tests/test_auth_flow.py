...
class AuthFlowTests(TestCase):
    ...
    def test_login_then_refresh_then_logout(self) -> None:
        User.objects.create_user(
            email="user@example.com", password="TestPass123!", phone="+491701234568"
        )
        ...
        refresh_response = self.client.post(
            "/api/v1/auth/login/refresh/", {"refresh": refresh}, format="json"
        )
        ...
        reuse_response = self.client.post(
            "/api/v1/auth/login/refresh/", {"refresh": rotated_refresh}, format="json"
        )
        self.assertEqual(reuse_response.status_code, 401)

    def test_login_with_wrong_password_rejected(self) -> None:
        User.objects.create_user(
            email="user2@example.com", password="TestPass123!", phone="+491701234569"
        )
        response = self.client.post(
            "/api/v1/auth/login/",
